    ...
    return (
        <nav className='px-4'>
            <ul className='flex items-center px-2 gap-2 lg:gap-4'>
                { menus.map((menu) => (
                    <li key={ menu.id } className={`px-2 lg:px-4 py-1 rounded ${location.pathname === menu.path ? 'md:bg-gradient-to-br from-primary to-primary-black hover:scale-105 shadow-inner text-primary-white-200' : ''}`}>
